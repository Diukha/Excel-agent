...
Создаёт компактное JSON-представление структуры Excel-файла,
оптимизированное для передачи в контекст LLM-агента.
"""
...
def analyze_excel(file_path: str, max_sample_rows: int = 3, max_unique_values: int = 5) -> dict:
    """Анализирует структуру Excel-файла и возвращает компактное описание.

    Args:
        file_path: Путь к Excel-файлу (.xlsx).
        max_sample_rows: Максимальное количество строк для примера данных.
        max_unique_values: Максимальное количество уникальных значений для отображения.

    Returns:
        Словарь с описанием структуры файла, включая листы, колонки и примеры данных.
    """
